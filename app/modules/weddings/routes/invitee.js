...
invitee.get("/create-slug", async function (req, res) {
	try {
		//create
		const invitees = await moduleModel.findAll(mod_config.collection, { status: 1, slug_name: { $in: ["", null] } });
		for (let index = 0; index < invitees.length; index++) {
			setTimeout(async () => {
				...
				moduleModel.updateOne(mod_config.collection, { _id: invitee?._id }, { slug_name: slug_name });
			}, index * 100);
		}
		req.flash("msg_success", "Create slug success");
		return helpers.base.redirect(res, mod_config.route);
	} catch (e) {
		req.flash("msg_error", e.message);
		return helpers.base.redirect(res, mod_config.route);
...
invitee.get("/create-link", async function (req, res) {
	try {
		//create
		const invitees = await moduleModel.findAll(mod_config.collection, { status: 1, link: { $in: ["", null] } });
		for (let index = 0; index < invitees.length; index++) {
			setTimeout(async () => {
				const invitee = invitees[index];
				...
				moduleModel.updateOne(mod_config.collection, { _id: invitee?._id }, { thumb: img, link: link, qr: qrgen?.name });
			}, index * 3000);
		}
		req.flash("msg_success", "Create slug success");
		return helpers.base.redirect(res, mod_config.route);
